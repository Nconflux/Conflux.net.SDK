...
namespace Nethereum.Accounts.IntegrationTests
{
    ...
    public class TransactionInterceptorTests
    {
        ...
        public async Task ShouldIntereceptAndSendRawTransaction()
        {
            ...
            var txId = await web3.Cfx.Transactions.SendTransaction.SendRequestAsync(
                new TransactionInput {From = senderAddress, To = receiveAddress, Value = new HexBigInteger(10)});

            var receipt = await web3.Cfx.Transactions.GetTransactionReceipt.SendRequestAsync(txId);
            while (receipt == null)
            {
                Thread.Sleep(1000);
                receipt = await web3.Cfx.Transactions.GetTransactionReceipt.SendRequestAsync(txId);
            }
        ...
        public async Task ShouldIntereceptContractDeploymentAndContractTrasanctionWithRawTransaction()
        {
            ...
            var txId = await web3.Cfx.DeployContract.SendRequestAsync(abi, contractByteCode, senderAddress,
