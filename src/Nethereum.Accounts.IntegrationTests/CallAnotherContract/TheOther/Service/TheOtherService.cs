...
namespace SolidityCallAnotherContract.Contracts.TheOther.Service
{
    ...
    public class TheOtherService
    {
        ...
        {
            return web3.Cfx.GetContractDeploymentHandler<TheOtherDeployment>().SendRequestAndWaitForReceiptAsync(theOtherDeployment, cancellationTokenSource);
        }
        public static Task<string> DeployContractAsync(Web3 web3, TheOtherDeployment theOtherDeployment)
        {
            return web3.Cfx.GetContractDeploymentHandler<TheOtherDeployment>().SendRequestAsync(theOtherDeployment);
        }
