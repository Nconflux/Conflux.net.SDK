...
namespace SolidityCallAnotherContract.Contracts.Test.Service
{
    ...
    public class TestService
    {
        ...
        {
            return web3.Cfx.GetContractDeploymentHandler<TestDeployment>().SendRequestAndWaitForReceiptAsync(testDeployment, cancellationTokenSource);
        }
        public static Task<string> DeployContractAsync(Web3 web3, TestDeployment testDeployment)
        {
            return web3.Cfx.GetContractDeploymentHandler<TestDeployment>().SendRequestAsync(testDeployment);
        }
