    ...
    public class NonceTests
    {
        ...
        public async void ShouldBeAbleToHandleNoncesOfMultipleTxnMultipleWeb3sMultithreaded()
        {
            ...
            Parallel.ForEach(taskItems, (item, state) =>
            {
                ...
                var txn = web3.Cfx.DeployContract.SendRequestAsync(abi, byteCode, senderAddress, new HexBigInteger(900000),
        ...
        public async void ShouldBeAbleToHandleNoncesOfMultipleTxnMultipleWeb3sSingleThreaded()
        {
            ...
            var txn1 = await
                web31.Cfx.DeployContract.SendRequestAsync(abi, byteCode, senderAddress, new HexBigInteger(900000), null,
                    multiplier);
            ...
            var txn2 = await
                web32.Cfx.DeployContract.SendRequestAsync(abi, byteCode, senderAddress, new HexBigInteger(900000), null,
                    multiplier);

            var web33 = new Web3.Web3(account, client);

            var txn3 = await
                web33.Cfx.DeployContract.SendRequestAsync(abi, byteCode, senderAddress, new HexBigInteger(900000), null,
                    multiplier);
        ...
        public async void ShouldBeAbleToHandleNoncesOfMultipleTxnSingleWeb3SingleThreaded()
        {
            ...
            var txn1 = await
                web3.Cfx.DeployContract.SendRequestAsync(abi, byteCode, senderAddress, new HexBigInteger(900000), null,
                    multiplier);
