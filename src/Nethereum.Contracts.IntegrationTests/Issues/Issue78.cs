...
namespace Nethereum.Contracts.IntegrationTests.Issues
{
    ...
    public class Issue78
    {
        ...
        public async Task ProdContract()
        {
            ...
            var contractHash = await web3.Cfx.DeployContract.SendRequestAsync(abi, byteCode, account,
                new HexBigInteger(900 * 1000), "My product");

            var receipt = await web3.Cfx.Transactions.GetTransactionReceipt.SendRequestAsync(contractHash);
            while (receipt == null)
            {
                await Task.Delay(1000);
                receipt = await web3.Cfx.Transactions.GetTransactionReceipt.SendRequestAsync(contractHash);
            }
