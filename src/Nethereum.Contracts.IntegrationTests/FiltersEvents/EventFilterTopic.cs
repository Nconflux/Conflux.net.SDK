...
namespace Nethereum.Contracts.IntegrationTests.FiltersEvents
{
    ...
    public class EventFilterTopic
    {
        ...
        public async Task StoreAndRetrieveStructs()
        {
            ...
            var accountBalance = await web3.Cfx.GetBalance.SendRequestAsync(account.Address);

            Assert.True(accountBalance.Value > 0);

            var receipt =
                await web3.Cfx.DeployContract.SendRequestAndWaitForReceiptAsync(
                    abi,
            ...
            var contract = web3.Cfx.GetContract(abi, contractAddress);
...
";
            ...
            var transactionHash = await web3.Cfx.DeployContract.SendRequestAsync(abi, contractByteCode, addressFrom, 7);
            ...
            while (receipt == null)
            {
                ...
                receipt = await web3.Cfx.Transactions.GetTransactionReceipt.SendRequestAsync(transactionHash);
            }

            var contract = web3.Cfx.GetContract(abi, receipt.ContractAddress);
            ...
            {
                await Task.Delay(5000);
                receiptTransaction = await web3.Cfx.Transactions.GetTransactionReceipt.SendRequestAsync(transaction7);
            }

            var logsAll = await web3.Cfx.Filters.GetFilterChangesForEthNewFilter.SendRequestAsync(filterAll);
            var logs69 = await web3.Cfx.Filters.GetFilterChangesForEthNewFilter.SendRequestAsync(filter69);
