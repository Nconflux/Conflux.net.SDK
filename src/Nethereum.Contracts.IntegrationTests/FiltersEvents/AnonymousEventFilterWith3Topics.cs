...
namespace Nethereum.Contracts.IntegrationTests.FiltersEvents
{
    ...
    public class AnonymousEventFilterWith3Topics
    {
        ...
        public async Task TestEvent()
        {
            ...
            var deploymentHandler = web3.Cfx.GetContractDeploymentHandler<TestAnonymousEventContractDeployment>();
            var deploymentTransactionReceipt = await deploymentHandler.SendRequestAndWaitForReceiptAsync(deploymentMessage);
            var contractHandler = web3.Cfx.GetContractHandler(deploymentTransactionReceipt.ContractAddress);
