...
using Conflux.ENS.Root.ContractDefinition;

namespace Conflux.ENS
{
    public partial class RootService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Conflux.Web3.Web3 web3, RootDeployment rootDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Cfx.GetContractDeploymentHandler<RootDeployment>().SendRequestAndWaitForReceiptAsync(rootDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Conflux.Web3.Web3 web3, RootDeployment rootDeployment)
        {
            return web3.Cfx.GetContractDeploymentHandler<RootDeployment>().SendRequestAsync(rootDeployment);
        }

        public static async Task<RootService> DeployContractAndGetServiceAsync(Conflux.Web3.Web3 web3, RootDeployment rootDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, rootDeployment, cancellationTokenSource);
        ...
        public RootService(Conflux.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
