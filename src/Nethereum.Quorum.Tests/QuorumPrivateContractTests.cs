    ...
    public class QuorumPrivateContractTests
    {
       ...
       /// private string url = "http://13.91.34.xxx";
        ...
        public async void PrivateRawTransactionTest()
        {
            ...
            var deploymentHandler = web3Quorum.Cfx.GetContractDeploymentHandler<StandardTokenDeployment>();
            //Deploying
            var transactionReceipt = await deploymentHandler.SendRequestAndWaitForReceiptAsync(deploymentMessage1);

            var contractAddress = transactionReceipt.ContractAddress;

            var totalSupply = await web3Quorum.Cfx.GetContractQueryHandler<TotalSupplyFunction>()
        ...
        public async void ShouldBeAbleToConnectTo7NodesPrivate()
        {
            ...
            var account = await web3Node1.Cfx.CoinBase.SendRequestAsync();
            var contract = web3Node1.Cfx.GetContract(abi, address);
        ...
        private static async Task<int> GetValue(string abi, string address, string nodeUrl)
        {
            ...
            var contract = web3.Cfx.GetContract(abi, address);
