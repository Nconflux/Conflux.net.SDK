...
using Conflux.JsonRpc.IpcClient;
using Conflux.JsonRpc.WebSocketClient;
using Conflux.JsonRpc.WebSocketStreamingClient;
using Conflux.RPC.Eth;
using Conflux.RPC.Eth.DTOs;
using Conflux.RPC.Reactive.Polling;
using Conflux.Web3.Accounts;
using Conflux.Web3.Accounts.Managed;

namespace Conflux.Parity.Reactive.ConsoleTest
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var res =  web3ws.Cfx.Blocks.GetBlockNumber.SendRequestAsync().Result; //task cancelled exception



            var clientipc = new IpcClient("jsonrpc.ipc");
            var web3ipc = new Web3.Web3(account, clientipc);
            var resIpc = web3ws.Cfx.Blocks.GetBlockNumber.SendRequestAsync().Result; 
            ...
            for (int i = 0; i < 10; i++)
            {
                web3.Cfx.GetEtherTransferService()
